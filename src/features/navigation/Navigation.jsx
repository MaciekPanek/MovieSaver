...
function Navigation() {
  return (
    <header className="w-screen sm:h-16 h-32 bg-stone-800 text-center flex flex-col sm:flex-row  justify-between items-center border-b-yellow-500 border-b-[3px]">
      <div className=" h-full w-full sm:w-min sm:px-4 ">
        <NavLink
          to={"/"}
          className={`${({ isActive }) =>
            isActive
              ? "active"
              : null} text-stone-300 text-xl md:text-2xl h-full flex items-center  justify-center sm:px-4 px-2 gap-2 border-b-[1px] border-b-yellow-300 sm:border-none `}
        >
          {" "}
